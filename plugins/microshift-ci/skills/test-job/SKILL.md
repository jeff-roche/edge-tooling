...
3. **Extracting MicroShift version** using the `extract-version.py` helper script from build logs
4. **Listing test scenarios** by fetching the scenario-info directory structure from GCS artifacts
5. **Analyzing test results** for each scenario using the `microshift-ci:test-scenario` command to get comprehensive JSON data
